...
- Evaluate feedback from ARD WG and community - when confident that no breaking changes will be necessary, publish the `1.x` GA stemcell on [bosh.io](https://bosh.io)
- Publish release notes and a migration guide alongside the `1.x` GA release

### App Runtime Deployments WG

Once the `0.x` alpha stemcell is validated by deploying a BOSH director and BATS are passing:

- Set up cf-deployment validation on GCP against the Resolute alpha stemcell
